/* ==========================================================================
   Interactive Engine for AutoVolt
   ========================================================================== */

// Base Simulated Article Database
const localArticles = [
    {
        "title": "Solid-State Battery Progress: Reaching the 600-Mile Range Milestone",
        "excerpt": "Battery manufacturers are transitioning solid-state cells to pilot manufacturing stages. We analyze the remaining engineering hurdles.",
        "category": "Battery Tech",
        "author": "Dominic Vance, Automotive Engineer",
        "date": "2 hours ago",
        "readTime": "5 min read",
        "body": "<p>Solid-state battery technology represents the next major milestone in vehicle electrification, offering double the energy density of conventional lithium-ion batteries and faster charging profiles.</p><p>Leading battery research groups are shifting their focus to manufacturing integrations. Laboratory prototypes have successfully completed hundreds of fast-charge cycles while maintaining capacity targets.</p><p>However, mass producing solid-state cells requires specialized cleanroom environments to protect the fragile ceramic separator layers, representing significant initial factory costs. Experts suggest that premium EVs will adopt solid-state packs by 2028, with mainstream models following as production scales.</p>",
        "premium": true
    },
    {
        "title": "Autonomous Driving Standards: Evolving Level-3 Approvals",
        "excerpt": "Automakers are obtaining regulatory clearances for eyes-off autonomous highway operation. We evaluate safety metrics.",
        "category": "Autonomy",
        "author": "Charles Vance",
        "date": "4 hours ago",
        "readTime": "6 min read",
        "body": "<p>Level 3 autonomy\u2014where the vehicle manages operations under specific highway conditions, allowing drivers to take their eyes off the road\u2014is gaining regulatory support.</p><p>Automakers are launching certified Level 3 systems in selected regions. The technology relies on a combination of LiDAR, radar, and optical cameras to construct real-time environmental models.</p><p>Legal liability guidelines are shifting, with manufacturers accepting responsibility for accidents that occur while autonomous systems are engaged, highlighting confidence in the software's safety capabilities.</p>",
        "premium": false
    },
    {
        "title": "EV Charging Infrastructure: Upgrading Highway Fast-Charging Corridors",
        "excerpt": "Discover how next-generation 350kW liquid-cooled fast chargers are reducing charging stops to under 15 minutes.",
        "category": "Infrastructure",
        "author": "Sarah Jenkins",
        "date": "1 day ago",
        "readTime": "4 min read",
        "body": "<p>Expanding access to reliable charging is key to widespread EV adoption. Public initiatives are deploying high-speed charging corridors along major interstate highways.</p><p>New 350kW liquid-cooled charging ports can supply up to 200 miles of range in under 15 minutes for compatible vehicle architectures. Improvements in payment systems and software integrations aim to simplify charging sessions for users.</p>",
        "premium": true
    }
];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (isArticlePage) {
        if (artId !== null) {
            ...
            const savedRssArt = localStorage.getItem('9_auto_volt_active_rss_article');
            const savedRssList = localStorage.getItem('9_auto_volt_rss_articles');
            if (savedRssList) {
...
function loadStateFromStorage() {
    const savedTheme = localStorage.getItem('9_auto_volt_dark_mode');
    state.isDarkMode = savedTheme === 'true';

    const savedBookmarks = localStorage.getItem('9_auto_volt_bookmarks');
    if (savedBookmarks) {
        state.bookmarks = JSON.parse(savedBookmarks);
    }

    const savedUpvotes = localStorage.getItem('9_auto_volt_upvotes');
    if (savedUpvotes) {
        state.upvoted = JSON.parse(savedUpvotes);
    }

    const savedComments = localStorage.getItem('9_auto_volt_comments');
    if (savedComments) {
        state.comments = JSON.parse(savedComments);
    }

    const savedSub = localStorage.getItem('9_auto_volt_subscribed');
    state.isSubscribed = savedSub === 'true';

    const savedViews = localStorage.getItem('9_auto_volt_pageviews');
    if (savedViews) state.pageViews = parseInt(savedViews);

    const savedClicks = localStorage.getItem('9_auto_volt_adclicks');
    if (savedClicks) state.adClicks = parseInt(savedClicks);

    const savedEarnings = localStorage.getItem('9_auto_volt_earnings');
...
function saveStateToStorage() {
    localStorage.setItem('9_auto_volt_dark_mode', state.isDarkMode);
    localStorage.setItem('9_auto_volt_bookmarks', JSON.stringify(state.bookmarks));
    localStorage.setItem('9_auto_volt_upvotes', JSON.stringify(state.upvoted));
    localStorage.setItem('9_auto_volt_comments', JSON.stringify(state.comments));
    localStorage.setItem('9_auto_volt_subscribed', state.isSubscribed);
    localStorage.setItem('9_auto_volt_pageviews', state.pageViews);
    localStorage.setItem('9_auto_volt_adclicks', state.adClicks);
    localStorage.setItem('9_auto_volt_earnings', state.earnings.toFixed(2));
}
...
function renderTicker() {
    ...
    const items = ["Solid-state battery cells demonstrate 620-mile range in laboratory tests", "National fast-charging corridor network reaches 50,000 active ports", "Level-3 autonomous software obtains highway operating approvals in selected regions", "Electric hypercar completes a record-setting lap at N\u00fcrburgring in 6m 12s", "EV battery recycling facilities report 95% material recovery rates"];
...
function handleArticleClick(article) {
    ...
    if (localIdx > -1) {
        ...
        localStorage.setItem('9_auto_volt_active_rss_article', JSON.stringify(article));
...
function runWorkspaceCalculator() {
    const resultEl = document.getElementById('calc-results');
            ...
            const miles = parseFloat(document.getElementById('annual-miles').value) || 12000;
            const mpg = parseFloat(document.getElementById('gas-mpg').value) || 25;
            const gasPrice = parseFloat(document.getElementById('gas-price-gal').value) || 3.50;
            const kwhCost = parseFloat(document.getElementById('electric-kwh').value) || 0.15;
            
            const gasTotalCost = (miles / mpg) * gasPrice;
            
            // Average EV gets ~3.5 miles per kWh
            const evKwhNeeded = miles / 3.5;
            const evTotalCost = evKwhNeeded * kwhCost;
            
            const annualSavings = gasTotalCost - evTotalCost;
            const monthlySavings = annualSavings / 12;
            
            resultEl.innerHTML = `
                <div class="calc-result-box" style="border-left: 4px solid var(--primary); padding-left: 10px; margin-top: 15px;">
                    <h5 style="margin: 0 0 5px 0;">Estimated Annual Savings:</h5>
                    <div style="font-size: 24px; font-weight: bold; color: var(--primary);">$${Math.round(annualSavings).toLocaleString()}<span style="font-size: 14px; font-weight: normal; color: var(--text-color);"> / year</span></div>
                    <div style="font-size: 11px; margin-top: 5px; opacity: 0.8;">Monthly Savings: <strong>$${Math.round(monthlySavings)} / mo</strong></div>
                    <div style="font-size: 11px;">Gas Cost: <del>$${Math.round(gasTotalCost)}</del> | EV Cost: <strong style="color: var(--primary);">$${Math.round(evTotalCost)}</strong></div>
                </div>
            `;
...
<a href="${link}" target="_blank" class="hero-btn" style="display:inline-block; margin-top:10px;">Read Original Source <i class="fa-solid fa-external-link"></i></a>`
            ...
            localStorage.setItem('9_auto_volt_rss_articles', JSON.stringify(parsedArticles));
